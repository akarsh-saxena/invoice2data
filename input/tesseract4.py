...
def to_text(path, language='fra'):
    ...
    # if not spawn.find_executable('gswin64c'):
    #     raise EnvironmentError('ghostscript not installed.')

    # print(path)

    # with tempfile.NamedTemporaryFile(suffix='.tiff') as tf:
    with open('C:\\Users\\Akarsh\\Desktop\\Good_Invoices\\temp.tiff') as tf:
        # print('tf.name: ', tf.name)
        # Step 1: Convert to TIFF
        gs_cmd = 'gswin64c -q -dNOPAUSE -r600x600 -sDEVICE=tiff24nc -sOutputFile=' + tf.name + ' ' + path + ' -c quit'
        print(gs_cmd)
        subprocess.Popen(gs_cmd, shell=True)
        time.sleep(3)
